...
def get_mahabs_distance(pool, training_length_dict, averaged_feature_vector, new_frequency_distribution, smoothing_factor):
    for str2 in pool:
        ...
        if(len(str2) in list(training_length_dict.keys())):
...
def train_and_test(training, test, attack_file, smoothing_factor, threshold_for_classification, verbose = "False"):
    ...
    all_ascii = [str(i) for i in range(256)]
    all_ascii_int = [ j for j in range(256)]
    ...
    for payload in training:
        ...
        if payload_length in list(training_length_dict.keys()):
    ...
    for key in sorted(training_length_dict.keys()):
        i = 0
    ...
    if len(attack[0]) not in list(training_length_dict.keys()):
    ...
    for str2 in attack:
        ...
        if mahabs_distance <= threshold_for_classification:            
            ...
            print("Calculated distance of {:.2f} is lesser than the threshold of {:d}. It fits the model." .format(mahabs_distance, threshold_for_classification))

        else:
            true_negative = true_negative + 1            
            print("Calculated distance of {:.2f} is greater than the threshold of {:d}. It doesn\'t fit the model." .format(mahabs_distance, threshold_for_classification))

    print('Total number of True Negatives: '+
          str((true_negative/float(len(attack)))*100.0))
    print('Total number of False Positives: '+
          str((false_positive/float(len(attack)))*100.0)+'\n')
